...
public class SerializationWriter 
{
	...
	public static int sendBytes(byte[] dest, int pointer, char value)
	{
		dest[pointer++] = (byte)((value >> 8) & 0xff);
		dest[pointer++] = (byte)((value >> 0) & 0xff);
		return pointer;
	...
	{
		return (int) (src[pointer] << 24 | src[pointer] << 16 | src[pointer + 2] << 8 | src[pointer + 3]);
	}
	
	public static long readLong(byte[] src, int pointer)
	{
		return (long) (src[pointer] << 56 | src[pointer + 1] << 48 | src[pointer + 2] << 40 | src[pointer + 3] | src[pointer] << 32 | 
